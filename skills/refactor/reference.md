...
- For interactive UI, would a **registry primitive** (e.g. from the **shadcn CLI**, a Radix-backed kit, etc.) replace bespoke overlay or list behavior? Am I about to **edit registry primitive files** for feature-only needs—if so, stop: wrap or compose from feature-owned code instead, unless the user asked for a design-system or primitive-layer change.
